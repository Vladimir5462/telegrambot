...
def start(message):
    conn = sqlite3.connect('testbot.sql')
    cur = conn.cursor()
    ...
    bot.send_message(message.chat.id, 'Проходит регистрация! Ввведите ваше имя, пожалуйста.')
    bot.register_next_step_handler(message, user_name)

def user_name(message):
    global name
    name = message.text.strip()
    bot.send_message(message.chat.id, 'Придумайте пароль, пожалуйста.')
